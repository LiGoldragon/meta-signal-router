...
const MODULE_PATH: &[&str] = &["meta_signal_router", "lib"];
const SIGNAL_MODULE_PATH: &[&str] = &["signal_router", "lib"];
...
const IMPORTED_STANDARD_TYPES: &[&str] = &["ComponentKind", "HostName", "NetworkEndpoint"];
...
impl SchemaBuild {
    ...
    fn run(&self) {
        ...
        let ordinary_metadata = CargoEthosSourceMetadata::new("signal-router");
        ordinary_metadata.emit_dependency_rerun_instruction();
        let ordinary_source_path = ordinary_metadata
            .dependency_source_directory()
            .expect("signal-router publishes its Ethos source directory")
            .join("interface.ethos");
        let ordinary_source = fs::read_to_string(&ordinary_source_path)
            .expect("read the producer-owned ordinary Router Interface source");
        assert_eq!(
            ordinary_source,
            signal_router::ROUTER_INTERFACE_SOURCE,
            "Cargo metadata must resolve the exact source compiled by signal-router",
        );
        ...
        let source = fs::read_to_string(&source_path).expect("read owner Router Interface source");
        ...
        .expect("assemble authority-approved owner Router Interface transaction");
        let rust = rust_logos();
        let type_paths = RouterRustTypePaths::new();

        BootstrapInterfaceGeneration::new(&assembly, &rust, &type_paths, &source_path, &rust_path)
            .generate()
            .expect("project owner Router Interface from the verified transaction")
            .write_or_check("META_SIGNAL_ROUTER_UPDATE_INTERFACE_ARTIFACTS")
            .expect("checked-in owner Router Interface source and Rust projection are fresh");
        CargoEthosSourceMetadata::new("meta-signal-router")
...
fn imported_signal_seats() -> Vec<&'static signal_router::bootstrap_manifest::DeclarationSeat> {
    let wanted = IMPORTED_SIGNAL_TYPES
        .iter()
        .copied()
        .collect::<BTreeSet<_>>();
    let seats = signal_router::bootstrap_manifest::DECLARATION_SEATS
        .iter()
...
fn bootstrap_catalog() -> BootstrapCatalog {
    ...
    .expect("owner Router bootstrap catalog is complete")
}
